...
首次安装会依次询问防火墙模式和邮件服务器域名，自动检测邮箱地址域名，再选择默认 `admin` 前缀或自行创建管理员邮箱前缀，最后输入密码并选择 Web 部署方式。防火墙可以选择自动添加邮局必要端口规则或保留现有规则，不会清空服务器已有防火墙。自动 Web 模式会把容器绑定到 `127.0.0.1:8088`，配置宿主机 Nginx，并使用官方 `acme.sh` 申请和续期证书。例如服务器域名 `mail.newszxcn.com`、选择默认前缀会创建 `admin@newszxcn.com`；自定义管理员密码最少 12 位，留空则生成 12 位密码。

安装或恢复成功后会集中显示邮箱登录地址、后台地址、邮局域名、管理员邮箱、安装时记录的密码和 `ns` 管理命令。对已有安装再次执行安装只会重建运行文件与容器，数据库、邮件、证书和配置保持原位；安装、修复、更新、回滚、重启、证书和卸载等修改操作共用更新锁。
