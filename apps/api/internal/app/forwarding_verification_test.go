...
func TestForwardingVerificationRateLimitAndTokenRotation(t *testing.T) {
	...
	if status := getStatusForTest(t, ts.URL+"/api/verify-email?token="+secondToken); status != http.StatusBadRequest {
		t.Fatalf("reused verified token status=%d", status)
	}
}
...
func TestForwardingVerificationAutomaticallyActivatesPendingMailboxBinding(t *testing.T) {
	...
	if err := a.db.QueryRow(`SELECT verification_token_hash FROM forwarding_verified_emails WHERE id=?`, verifiedID).Scan(&tokenHash); err != nil || tokenHash != "" {
