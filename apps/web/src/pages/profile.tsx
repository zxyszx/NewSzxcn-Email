...
function MailboxManagement({
  ...
  const { toast } = useToast()
  const isMobile = useIsMobile()
  const [mailboxViewParams, setMailboxViewParams] = useSearchParams()
  const rawMailboxView = mailboxViewParams.get("mailboxView")
  const mailboxView: MailboxView = rawMailboxView === "forwarding" || rawMailboxView === "verified" ? rawMailboxView : "mine"
  ...
  const [mailboxPage, setMailboxPage] = React.useState(1)
  const [mailboxPageSize, setMailboxPageSize] = React.useState(10)
  const [singleForwardSearch, setSingleForwardSearch] = React.useState("")
  const [singleForwardPage, setSingleForwardPage] = React.useState(1)
  ...
  const [mailboxBindingDraft, setMailboxBindingDraft] = React.useState("")
  const [accountBindingDraft, setAccountBindingDraft] = React.useState("")
  ...
  const failedVerifiedEmailItems = React.useMemo(() => verifiedEmailItems
    .filter((item) => !item.verified && item.deliveryStatus === "failed")
    .sort((a, b) => forwardingItemTime(b, false) - forwardingItemTime(a, false)), [verifiedEmailItems])
  const pendingVerifiedEmailItems = React.useMemo(() => verifiedEmailItems
    .filter((item) => !item.verified && item.deliveryStatus !== "failed")
    .sort((a, b) => forwardingItemTime(b, false) - forwardingItemTime(a, false)), [verifiedEmailItems])
  const completedVerifiedEmailItems = React.useMemo(() => verifiedEmailItems
    .filter((item) => item.verified)
    .sort((a, b) => forwardingItemTime(b, true) - forwardingItemTime(a, true)), [verifiedEmailItems])
  const normalizedVerifiedSearch = verifiedSearch.trim().toLowerCase()
  const matchingPendingVerifiedEmailItems = React.useMemo(() => normalizedVerifiedSearch
    ? pendingVerifiedEmailItems.filter((item) => item.email.toLowerCase().includes(normalizedVerifiedSearch))
    : pendingVerifiedEmailItems, [normalizedVerifiedSearch, pendingVerifiedEmailItems])
  const matchingFailedVerifiedEmailItems = React.useMemo(() => normalizedVerifiedSearch
    ? failedVerifiedEmailItems.filter((item) => item.email.toLowerCase().includes(normalizedVerifiedSearch))
    : failedVerifiedEmailItems, [failedVerifiedEmailItems, normalizedVerifiedSearch])
  const matchingCompletedVerifiedEmailItems = React.useMemo(() => normalizedVerifiedSearch
    ? completedVerifiedEmailItems.filter((item) => item.email.toLowerCase().includes(normalizedVerifiedSearch))
    : completedVerifiedEmailItems, [completedVerifiedEmailItems, normalizedVerifiedSearch])
  const verifiedMatchCount = matchingFailedVerifiedEmailItems.length + matchingPendingVerifiedEmailItems.length + matchingCompletedVerifiedEmailItems.length
  const verifiedSearchExactItem = verifiedEmailItems.find((item) => item.email.toLowerCase() === normalizedVerifiedSearch)
  const canAddVerifiedSearch = looksLikeEmailAddress(verifiedSearch) && !verifiedSearchExactItem
  const hasPendingVerifiedEmails = pendingVerifiedEmailItems.length > 0
  const pendingBindings = React.useMemo(() => (forwarding.data?.pendingBindings || []).filter((item) => item.status !== "active" && item.status !== "cancelled"), [forwarding.data?.pendingBindings])
  const accountPendingBindings = React.useMemo(() => pendingBindings.filter((item) => item.scope === "account"), [pendingBindings])
  const mailboxPendingBindings = React.useMemo(() => forwardingMailbox ? pendingBindings.filter((item) => item.scope === "mailbox" && item.mailboxId === forwardingMailbox.id) : [], [forwardingMailbox, pendingBindings])
  ...
  const savedAccountForwardTargets = forwardingTargetsFromSettings(forwarding.data)
  const accountForwardingChanged = accountForwardTargets.join("\n") !== savedAccountForwardTargets.join("\n")
  ...
  const addVerifiedEmail = useMutation({
    ...
    onSuccess: (settings, email) => {
      ...
      toast({
        title: item?.deliveryStatus === "failed" ? "验证邮箱已添加，邮件发送失败" : "验证邮件已加入发送队列",
        description: item?.deliveryStatus === "failed" ? item.deliveryError || "请稍后重发验证邮件" : "发送后请检查目标邮箱的收件箱和垃圾邮件",
      })
    },
  ...
  const resendVerifiedEmail = useMutation({
    ...
    onSuccess: (settings, item) => {
      ...
      toast({
        title: next?.deliveryStatus === "failed" ? "重发失败" : "验证邮件已重新加入发送队列",
        description: next?.deliveryStatus === "failed" ? next.deliveryError || "请稍后再试" : "发送后请检查目标邮箱的收件箱和垃圾邮件",
      })
    },
  ...
  const createPendingBinding = useMutation({
    mutationFn: api.createForwardingPendingBinding,
    onSuccess: (settings, variables) => {
      setForwardingCache(settings)
      refreshForwardingSettings()
      window.setTimeout(refreshForwardingSettings, 2500)
      window.setTimeout(refreshForwardingSettings, 7000)
      const binding = (settings.pendingBindings || []).find((item) => item.scope === variables.scope && item.email.toLowerCase() === variables.email.trim().toLowerCase() && (variables.scope === "account" || item.mailboxId === variables.mailboxId))
      if (variables.scope === "account") setAccountBindingDraft("")
      else setMailboxBindingDraft("")
      toast({
        title: binding?.status === "pending_verification" ? "等待对方验证" : "转发目标已绑定",
        description: binding?.status === "pending_verification" ? `验证邮件已发送至 ${variables.email}，验证成功后会自动绑定。` : `${variables.email} 已加入转发目标。`,
      })
    },
    onError: (error) => toast({ title: "绑定失败", description: error.message }),
  })
  const cancelPendingBinding = useMutation({
    mutationFn: api.deleteForwardingPendingBinding,
    onSuccess: (settings) => {
      setForwardingCache(settings)
      toast({ title: "已取消等待绑定" })
    },
  ...
  const retryPendingBinding = useMutation({
    mutationFn: api.retryForwardingPendingBinding,
    onSuccess: (settings) => {
      setForwardingCache(settings)
      toast({ title: "已重新尝试自动绑定" })
    },
  ...
  React.useEffect(() => {
    if (!hasPendingVerifiedEmails && pendingBindings.length === 0) return
    const timer = window.setInterval(() => { void forwarding.refetch() }, mailboxView === "verified" ? 3000 : 10000)
    return () => window.clearInterval(timer)
  }, [forwarding, hasPendingVerifiedEmails, mailboxView, pendingBindings.length])
  ...
  function setMailboxView(next: MailboxView) {
    ...
    if (next === "mine") nextParams.delete("mailboxView")
    else nextParams.set("mailboxView", next)
    setMailboxViewParams(nextParams)
  }
  ...
  function submitVerifiedSearch(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const value = verifiedSearch.trim().toLowerCase()
    if (!looksLikeEmailAddress(value) || verifiedSearchExactItem) return
  ...
  function bindForwardingTarget(scope: "account" | "mailbox") {
    const email = (scope === "account" ? accountBindingDraft : mailboxBindingDraft).trim().toLowerCase()
    if (!looksLikeEmailAddress(email)) {
      toast({ title: "请输入有效的邮箱地址" })
      return
    }
    if (scope === "mailbox" && !forwardingMailbox) return
    createPendingBinding.mutate({ email, scope, ...(scope === "mailbox" ? { mailboxId: forwardingMailbox!.id } : {}) })
  }
  ...
  return (
    <div className="profile-mailbox-sections space-y-1">
      <section className="overflow-hidden rounded-lg border bg-card">
        ...
        {mailboxView === "mine" && (
          <div className="grid grid-cols-3 gap-2 border-t p-2.5 sm:p-3">
            <MailboxStatCard icon={<Mail className="h-5 w-5" />} label="邮箱总数" value={mailboxes.length} tone="primary" />
            <MailboxStatCard icon={<MailCheck className="h-5 w-5" />} label="已验证地址" value={forwarding.isLoading ? "--" : completedVerifiedEmailItems.length} tone="success" />
            <MailboxStatCard icon={<SendHorizontal className="h-5 w-5" />} label="启用转发" value={forwarding.isLoading ? "--" : forwardedMailboxCount} tone="violet" />
          </div>
        )}
        {mailboxView === "verified" && (
          <div className="grid grid-cols-3 gap-2 border-t p-2.5 sm:p-3">
            <MailboxStatCard icon={<MailCheck className="h-5 w-5" />} label="已验证" value={completedVerifiedEmailItems.length} tone="success" />
            <MailboxStatCard icon={<Clock3 className="h-5 w-5" />} label="待验证" value={pendingVerifiedEmailItems.length} tone="warning" />
            <MailboxStatCard icon={<CheckCircle2 className="h-5 w-5" />} label="最近验证" value={latestVerifiedAt ? new Date(latestVerifiedAt).toLocaleDateString() : "--"} tone="violet" compact />
          </div>
        )}
      </section>
      ...
      <section className="rounded-lg border bg-card">
          ...
          {pagedMailboxes.map((mailbox) => {
            ...
            return (
              <div key={mailbox.id} className={cn("grid gap-2 px-4 py-3 sm:px-5 md:grid-cols-[minmax(0,1fr)_minmax(220px,0.8fr)_100px] md:items-center md:gap-4", selectedMailboxId === mailbox.id && "bg-muted/50")}>
                ...
                <div className="min-w-0 space-y-1 pl-12 text-xs text-muted-foreground md:pl-0">
                  <div className="tabular-nums">创建于 {formatDateTime(mailbox.createdAt)}</div>
        ...
        <PaginationControls page={mailboxPage} pageSize={mailboxPageSize} total={filteredMailboxes.length} onPageChange={setMailboxPage} onPageSizeChange={setMailboxPageSize} />
      </section>
      </>}

      {mailboxView === "forwarding" && <section id="mailbox-forwarding-section" className="rounded-lg border bg-card px-4 py-3 sm:px-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h2 className="text-lg font-semibold leading-7">邮件转发</h2>
          <span className="text-xs text-muted-foreground">验证后自动绑定</span>
        </div>
        <div className="rounded-lg border bg-muted/15 p-3 sm:p-4">
          ...
          <PendingBindingList items={accountPendingBindings} busy={forwardingBusy} onCancel={(id) => cancelPendingBinding.mutate(id)} onRetry={(id) => retryPendingBinding.mutate(id)} />
          {verifiedEmails.length > 0 && <div className="mt-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_72px] md:items-start">
            ...
            <Button type="button" variant="outline" className="h-11 md:h-[37px]" disabled={forwardingBusy || !accountForwardingChanged} onClick={() => saveAccountForwarding.mutate(accountForwardTargets)}>{saveAccountForwarding.isPending ? "保存中" : accountForwardingChanged ? "保存" : "已保存"}</Button>
          </div>}
        </div>
        <p className="mt-2 text-xs text-muted-foreground">点击“我的邮箱”列表中的「转发」，可为单个邮箱追加独立目标。</p>
        ...
        <div className="mt-3 border-t pt-3">
          ...
          <div className="mt-4 overflow-hidden rounded-lg border">
            <div className="hidden grid-cols-[minmax(0,1fr)_minmax(180px,0.7fr)_88px] gap-4 border-b bg-muted/25 px-4 py-2 text-xs font-medium text-muted-foreground sm:grid">
              <span>邮箱账号</span>
              <span>转发范围</span>
              <span className="text-right">操作</span>
            </div>
            ...
            {pagedSingleForwardMailboxes.map((mailbox) => (
              <div key={mailbox.id} className="grid gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_minmax(180px,0.7fr)_88px] sm:items-center sm:gap-4">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white"><Mail className="h-4 w-4" /></span>
                  <div className="min-w-0 break-all text-sm font-semibold">{mailbox.address}</div>
                </div>
                <div className="pl-12 text-xs leading-5 text-muted-foreground sm:pl-0">单独目标 {(mailboxForwards[mailbox.id] || []).length} 个{accountForwardTargets.length > 0 ? ` · 继承账号级 ${accountForwardTargets.length} 个` : ""}</div>
                <Button type="button" variant="outline" className="h-11 w-full sm:h-9 sm:w-[88px]" onClick={() => openMailboxForward(mailbox)}>管理</Button>
              </div>
            ))}
      ...
      {mailboxView === "mine" && externalImapEnabled && (
        <section className="rounded-lg border bg-card">
      ...
      {mailboxView === "verified" && (
        <section className="rounded-lg border bg-card">
          ...
          {normalizedVerifiedSearch && (
            <div className="border-b px-4 py-2 text-xs text-muted-foreground sm:px-5" aria-live="polite">
              {verifiedSearchExactItem ? `该邮箱已${verifiedSearchExactItem.verified ? "验证" : "添加"}，无需重复添加。` : canAddVerifiedSearch ? "该邮箱尚未添加，可点击右侧“添加邮箱”发送验证邮件。" : `找到 ${verifiedMatchCount} 个匹配邮箱`}
            </div>
          )}

          <div className="space-y-2 px-4 py-2.5 sm:px-5">
            {normalizedVerifiedSearch && verifiedMatchCount === 0 && (
              <div className="rounded-lg border border-dashed px-5 py-8 text-center text-sm text-muted-foreground">没有已添加的匹配邮箱</div>
            )}
            {matchingFailedVerifiedEmailItems.length > 0 && (
              <div>
            ...
            {matchingPendingVerifiedEmailItems.length > 0 && (
              <div>
                <div className="mb-1.5 flex items-center gap-2">
            ...
            {matchingCompletedVerifiedEmailItems.length > 0 ? <div>
              <div className="mb-1.5 flex items-center gap-2">
              ...
              <div className="overflow-hidden rounded-lg border">
                ...
                {pagedVerifiedEmailItems.map((item) => (
                  <VerifiedEmailRow
                    key={item.id}
                    item={item}
                    busy={deleteVerifiedEmail.isPending}
                    mailboxCount={mailboxes.length}
                    onResend={resendVerification}
                    onRemove={() => setPendingVerifiedDelete(item)}
                    onViewBindings={() => {
                      const targets = item.globalBinding ? sortedMailboxes.map((mailbox) => mailbox.address) : sortForwardingTargets(item.mailboxBindings || [])
                      if (targets.length === 0) return
                      setForwardingTargetPreview({
                        title: item.email,
                        subtitle: item.globalBinding ? `账号级转发绑定全部 ${targets.length} 个邮箱` : `已绑定 ${targets.length} 个单邮箱转发规则`,
                        source: item.globalBinding ? "全部邮箱" : "单邮箱",
                        targets,
                      })
                    }}
                  />
              ...
              <PaginationControls page={verifiedPage} pageSize={verifiedPageSize} total={matchingCompletedVerifiedEmailItems.length} onPageChange={setVerifiedPage} onPageSizeChange={setVerifiedPageSize} />
            </div> : !normalizedVerifiedSearch && completedVerifiedEmailItems.length === 0 ? (
              <div className="rounded-lg border border-dashed px-5 py-6 text-center">
                <MailCheck className="mx-auto h-7 w-7 text-muted-foreground" />
                <p className="mt-3 text-sm font-medium">还没有验证邮箱</p>
                <p className="mt-1 text-sm text-muted-foreground">添加并验证后，才能作为邮件转发目标。</p>
              </div>
            ) : null}
          </div>
        </section>
      ...
      {isMobile ? (
        <Sheet open={!!forwardingMailbox} onOpenChange={(open) => { if (!open) setForwardingMailbox(null) }}>
          <SheetContent side="bottom" className="mailbox-sheet-content flex max-h-[92dvh] min-h-0 flex-col overflow-hidden rounded-t-xl p-0 pb-[calc(1rem+env(safe-area-inset-bottom))]">
            ...
            <div className="min-h-0 overflow-y-auto px-5 py-5">
              <div className="space-y-3">
                <DirectForwardingBinding
                  value={mailboxBindingDraft}
                  onChange={setMailboxBindingDraft}
                  items={mailboxPendingBindings}
                  busy={forwardingBusy}
                  onBind={() => bindForwardingTarget("mailbox")}
                  onCancel={(id) => cancelPendingBinding.mutate(id)}
                  onRetry={(id) => retryPendingBinding.mutate(id)}
                />
      ...
      <Dialog open={!!forwardingMailbox} onOpenChange={(open) => { if (!open) setForwardingMailbox(null) }}>
        <DialogContent className="forwarding-dialog w-[calc(100vw-1.5rem)] max-w-none sm:max-w-lg">
          ...
          <DialogFooter className="gap-2 sm:gap-2">
            <Button type="button" variant="outline" className="h-10" disabled={forwardingBusy} onClick={() => setForwardingMailbox(null)}>取消</Button>
            <Button type="button" className="h-10" disabled={forwardingBusy} onClick={saveMailboxForward}>{saveMailboxForwarding.isPending ? "保存中" : "保存"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      )}
      ...
      <Sheet open={mobileCreateOpen} onOpenChange={setMobileCreateOpen}>
        <SheetContent side="bottom" className="mailbox-sheet-content max-h-[85dvh] overflow-y-auto rounded-t-xl pb-[calc(1rem+env(safe-area-inset-bottom))]">
          <SheetHeader className="text-left"><SheetTitle>创建新邮箱</SheetTitle></SheetHeader>
          <MailboxCreateForm
            localPart={localPart}
      ...
      <ConfirmDialog
        open={!!pendingVerifiedDelete}
        title="删除验证邮箱？"
        description={pendingVerifiedDelete ? `删除 ${pendingVerifiedDelete.email} 后，它将不能继续作为转发目标。` : undefined}
        confirmText="删除"
        destructive
        pending={deleteVerifiedEmail.isPending}
        onOpenChange={(open) => { if (!open) setPendingVerifiedDelete(null) }}
        onConfirm={() => {
          if (!pendingVerifiedDelete) return
          removeVerifiedEmail(pendingVerifiedDelete.id, pendingVerifiedDelete.email)
        }}
      />
...
function MailboxViewTabs({ value, onChange, pendingCount }: { value: MailboxView; onChange: (view: MailboxView) => void; pendingCount: number }) {
  const items: { key: MailboxView; label: string }[] = [
    { key: "mine", label: "我的邮箱" },
    { key: "forwarding", label: "邮件转发" },
    { key: "verified", label: "验证邮箱" },
  ]
  return (
    <div className="grid grid-cols-3 px-2 sm:px-4" role="tablist" aria-label="邮箱管理视图">
      {items.map((item) => (
        <Button
          key={item.key}
          type="button"
          ...
          className={cn(
            "relative flex min-h-12 rounded-none px-1 text-sm font-medium text-muted-foreground transition-colors hover:bg-transparent hover:text-foreground sm:px-5",
            value === item.key && "text-primary",
          )}
          onClick={() => onChange(item.key)}
        >
          <span>{item.label}</span>
          {item.key === "verified" && pendingCount > 0 && <span className="min-w-5 rounded-full bg-amber-500 px-1.5 py-0.5 text-[10px] font-semibold leading-4 text-white">{pendingCount}</span>}
          {value === item.key && <span className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-primary sm:inset-x-8" />}
        </Button>
      ))}
    </div>
  )
...
function MailboxStatCard({ icon, label, value, tone, compact }: { icon: React.ReactNode; label: string; value: string | number; tone: "primary" | "success" | "warning" | "violet"; compact?: boolean }) {
  const toneClass = {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 rounded-lg border bg-background/55 px-2 py-3 sm:gap-3 sm:px-4">
      <span className={cn("hidden size-10 shrink-0 items-center justify-center rounded-full sm:flex", toneClass)}>{icon}</span>
      <span className="min-w-0">
        <span className="block text-[11px] leading-4 text-muted-foreground sm:text-xs">{label}</span>
        <span className={cn("mt-0.5 block truncate font-semibold tabular-nums", compact ? "text-sm sm:text-base" : "text-lg sm:text-xl")}>{value}</span>
      </span>
    </div>
  )
}

function PaginationControls({ page, pageSize, total, onPageChange, onPageSizeChange }: { page: number; pageSize: number; total: number; onPageChange: (page: number) => void; onPageSizeChange?: (pageSize: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground sm:justify-start">
        <span>{firstItem}-{lastItem} / {total}</span>
        {onPageSizeChange && (
          <div className="flex items-center gap-2">
            <span>每页</span>
            <Select value={String(pageSize)} onValueChange={(value) => onPageSizeChange(Number(value))}>
              <SelectTrigger className="h-9 w-[68px] text-xs" aria-label="每页邮箱数量"><SelectValue /></SelectTrigger>
      ...
      <div className="grid grid-cols-2 gap-2 sm:flex">
        <Button type="button" variant="outline" size="sm" className="h-11 sm:h-9" disabled={page <= 1} onClick={() => onPageChange(page - 1)}>上一页</Button>
        <Button type="button" variant="outline" size="sm" className="h-11 sm:h-9" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)}>下一页</Button>
      </div>
    </div>
  )
}

function MailboxCreateForm({ localPart, onLocalPartChange, selectedDomainId, domainOptions, onDomainChange, canApply, pending, onSubmit }: {
  localPart: string
  onLocalPartChange: (value: string) => void
  selectedDomainId: string
  domainOptions: MailboxApplyOptions["domains"]
  ...
  return (
    <form className="mt-5 space-y-4" onSubmit={onSubmit}>
      <div className="space-y-2">
        <Label htmlFor="mobile-mailbox-prefix">邮箱地址前缀</Label>
        <Input id="mobile-mailbox-prefix" value={localPart} onChange={(event) => onLocalPartChange(event.target.value)} className="h-11 text-base" placeholder="例如 netflix01" disabled={!canApply || pending} />
      </div>
      <div className="space-y-2">
        <Label>邮箱域名</Label>
        <Select value={selectedDomainId} onValueChange={onDomainChange} disabled={!canApply || pending || domainOptions.length === 0}>
          <SelectTrigger className="h-11 text-base" aria-label="邮箱域名"><SelectValue placeholder="暂无域名" /></SelectTrigger>
          <SelectContent>{domainOptions.map((domain) => <SelectItem key={domain.id} value={domain.id}>@{domain.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <p className="text-sm leading-5 text-muted-foreground">邮箱数量受账号配额限制，创建成功后会自动刷新列表。</p>
      <Button className="h-11 w-full" disabled={!canApply || pending || !selectedDomainId || !localPart.trim()}>{pending ? "创建中" : "创建邮箱"}</Button>
    </form>
  )
}

function DirectForwardingBinding({ value, onChange, items, busy, onBind, onCancel, onRetry }: {
  value: string
  onChange: (value: string) => void
  items: ForwardingPendingBinding[]
  busy: boolean
  onBind: () => void
  ...
  return (
    <div className="space-y-2">
      <Label className="text-sm font-medium">绑定新的转发邮箱</Label>
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_76px]">
        <Input
          ...
          onKeyDown={(event) => { if (event.key === "Enter") onBind() }}
          className="h-11 text-base sm:h-10 sm:text-sm"
          placeholder="输入外部邮箱地址"
          disabled={busy}
        />
        <Button type="button" className="h-11 sm:h-10" disabled={busy || !value.trim()} onClick={onBind}>绑定</Button>
      </div>
      <p className="text-xs leading-5 text-muted-foreground">已验证地址立即绑定；未验证地址会先发送验证邮件，确认后自动绑定。</p>
      <PendingBindingList items={items} busy={busy} onCancel={onCancel} onRetry={onRetry} />
    </div>
  )
}

function PendingBindingList({ items, busy, onCancel, onRetry }: {
  items: ForwardingPendingBinding[]
  busy: boolean
  onCancel: (id: string) => void
  onRetry: (id: string) => void
  ...
  return (
    <div className="overflow-hidden rounded-md border bg-amber-50/50 dark:bg-amber-500/5">
      {items.map((item) => {
        const failed = item.status === "activation_failed"
        ...
        return (
          <div key={item.id} className="flex min-h-11 flex-wrap items-center gap-2 border-b px-3 py-2 text-xs last:border-b-0">
            ...
            {failed && <Button type="button" variant="ghost" size="sm" className="h-8 px-2 text-xs" disabled={busy} onClick={() => onRetry(item.id)}>重试</Button>}
            {!expired && <Button type="button" variant="ghost" size="sm" className="h-8 px-2 text-xs text-destructive hover:text-destructive" disabled={busy} onClick={() => onCancel(item.id)}>取消</Button>}
          </div>
        )
      })}
    </div>
...
function forwardingItemTime(item: ForwardingVerifiedEmail, verified: boolean) {
  const value = verified ? item.verifiedAt || item.createdAt : item.verificationSentAt || item.createdAt
  ...
  return (
    <div className="relative min-w-0">
      <Button
        ...
        variant="outline"
        className={cn(
          "h-auto min-h-11 w-full justify-between gap-3 bg-background px-3 py-2 text-left text-sm font-normal hover:bg-accent/60 hover:text-foreground",
      ...
      >
        <span className="min-w-0 flex-1">
          <span className={cn("block truncate", selectedEmails.length ? "font-medium text-foreground" : "text-muted-foreground")}>{label}</span>
          {selectedEmails.length > 0 && <span className="mt-0.5 block truncate text-xs font-normal text-muted-foreground" title={selectedSummary}>{selectedSummary}</span>}
        </span>
      ...
      {open && (
        <div className={cn(
          ...
          <div className="flex items-center justify-between border-b px-3 py-2">
            <span className="text-sm font-semibold">选择已验证邮箱</span>
          ...
          <div className="p-2">
            <div className="relative min-w-0">
              ...
              />
              {query && (
                <Button type="button" variant="ghost" size="icon" className="absolute right-0 top-0 size-11 text-muted-foreground" aria-label="清除搜索" title="清除搜索" onClick={() => setQuery("")}>
                  <X className="h-3.5 w-3.5" />
                </Button>
              )}
            </div>
          </div>
          <div className="max-h-[240px] overflow-y-auto border-t p-2">
            <div className="space-y-1" role="listbox" aria-label="已验证邮箱账号" aria-multiselectable="true">
              {filteredLockedEmails.map((email) => (
                <label
                  key={`locked-${email}`}
                  className="flex min-h-11 cursor-not-allowed items-center gap-2 rounded-md border border-border bg-muted/70 px-2.5 py-2 text-sm font-semibold text-muted-foreground"
                >
              ...
              {filteredEmails.map((email) => {
                ...
                return (
                  <label
                    key={email}
                    className={cn(
                      "flex min-h-11 cursor-pointer items-center gap-2 rounded-md border border-transparent px-2.5 py-2 text-sm transition-colors hover:border-border hover:bg-muted/70",
          ...
          <div className="flex items-center justify-between gap-3 border-t px-3 py-2">
            <span className="text-xs text-muted-foreground">显示 {filteredLockedEmails.length + filteredEmails.length} 个邮箱</span>
            <div className="flex items-center gap-2">
              <Button type="button" variant="ghost" size="sm" className="h-9 px-2 text-xs text-muted-foreground" disabled={selectedEmailsOnly.length === 0} onClick={() => onChange([])}>清空选择</Button>
              <Button type="button" size="sm" className="h-9 min-w-20" onClick={() => setOpen(false)}>完成</Button>
            </div>
          </div>
        </div>
      )}
...
function VerifiedEmailListHeader() {
  return (
    <div className="hidden grid-cols-[minmax(0,1.1fr)_minmax(220px,0.9fr)_auto] gap-4 border-b bg-muted/25 px-4 py-2 text-xs font-medium text-muted-foreground sm:grid">
      <span>验证邮箱</span>
      <span>状态与转发绑定</span>
      <span className="min-w-[9rem] text-right">操作</span>
    </div>
  )
}

function VerifiedEmailRow({ item, busy, mailboxCount = 0, onResend, onRemove, onViewBindings }: {
  ...
  const sentAt = item.verificationSentAt ? Date.parse(item.verificationSentAt) : 0
  const resendWaitSeconds = item.verified || !Number.isFinite(sentAt) ? 0 : Math.max(0, Math.ceil((sentAt + 60_000 - clock) / 1000))
  React.useEffect(() => {
    if (item.verified || !Number.isFinite(sentAt) || sentAt <= 0 || sentAt + 60_000 <= Date.now()) return
    const timer = window.setInterval(() => setClock(Date.now()), 1000)
    return () => window.clearInterval(timer)
  }, [item.verified, sentAt])
  const mailboxBindings = sortForwardingTargets(item.mailboxBindings || [])
  const bindingCount = item.globalBinding ? mailboxCount : mailboxBindings.length
  const bindingLabel = item.globalBinding
    ? `转发绑定：全部邮箱（${mailboxCount} 个）`
    : bindingCount > 0
      ? `转发绑定：${bindingCount} 个邮箱${bindingCount <= 2 ? ` · ${mailboxBindings.join("、")}` : ""}`
      : "未用于邮件转发"
  return (
    <div className="grid min-h-[68px] gap-2 border-b px-4 py-3 last:border-b-0 sm:grid-cols-[minmax(0,1.1fr)_minmax(220px,0.9fr)_auto] sm:items-center sm:gap-4">
      <div className="flex min-w-0 items-start gap-2 sm:items-center">
        <span className={cn("mt-1.5 size-2 shrink-0 rounded-full sm:mt-0", tone.dotClass)} />
      ...
      <div className="min-w-0 pl-4 sm:pl-0">
        <div className={cn("text-xs leading-5 tabular-nums", item.verified ? "text-muted-foreground" : tone.detailClass)}>
          {item.verified ? `验证于 ${formatDateTime(item.verifiedAt || item.createdAt)}` : forwardingEmailStatusText(item)}
        </div>
        {item.verified && (
          onViewBindings && bindingCount > 0 ? (
            <Button type="button" variant="link" className="mt-0.5 h-auto max-w-full justify-start truncate p-0 text-xs font-normal" onClick={onViewBindings} title={bindingLabel}>{bindingLabel}</Button>
          ) : (
            <div className="mt-0.5 text-xs text-muted-foreground/80">{bindingLabel}</div>
          )
        )}
      </div>
      <div className="flex min-w-[9rem] shrink-0 items-center justify-end gap-2 pl-4 sm:pl-0">
        {!item.verified && (
          <Button type="button" variant="outline" size="sm" className="h-11 min-w-[5.5rem] sm:h-9" disabled={busy || resendWaitSeconds > 0} onClick={() => onResend(item)}>{resendWaitSeconds > 0 ? `${resendWaitSeconds}s 后重发` : "重发"}</Button>
        )}
        <Button type="button" variant="ghost" size="sm" className="h-11 gap-1 text-destructive hover:bg-destructive/10 hover:text-destructive sm:h-9" disabled={busy} onClick={() => onRemove(item.id, item.email)} aria-label={`移除 ${item.email}`}><Trash2 className="h-3.5 w-3.5" />移除</Button>
      </div>
    </div>
  )
}
...
function forwardingEmailStatusText(item: ForwardingVerifiedEmail) {
  ...
  if (item.deliveryStatus === "delivered") {
    return `验证邮件已交给邮件服务器，请检查收件箱和垃圾邮件${time}`
  }
